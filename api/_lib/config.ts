 ...
 *   - functions/src/config.ts    (only if you keep the old Cloud Functions around)
 *
 * Only one source of truth is not possible across Firestore rules (which run
 * in a sandboxed language) and the app code, so this constant exists in
 * multiple places on purpose. If you ever change the admin email, update all.
 */
